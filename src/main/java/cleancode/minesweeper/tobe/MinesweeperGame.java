...
public class MinesweeperGame {
    ...
    private static String[][] BOARD = new String[BOARD_COL_SIZE][BOARD_ROW_SIZE];
    private static Integer[][] NEARBY_LAND_MINE_COUNTS = new Integer[BOARD_COL_SIZE][BOARD_ROW_SIZE];
    private static boolean[][] LAND_MINES = new boolean[BOARD_COL_SIZE][BOARD_ROW_SIZE];
    ...
    private static boolean checkIfAllCellIsOpened() {
        ...
        for (int row = 0; row < BOARD_COL_SIZE; row++) {
        ...
        for (int row = 0; row < BOARD_COL_SIZE; row++) {
            for (int col = 0; col < BOARD_ROW_SIZE; col++) {
                ...
                if (!isLandMineCell(col, row)) {
                    ...
                    if (row - 1 >= 0 && col + 1 < BOARD_ROW_SIZE && isLandMineCell(col + 1, row - 1)) {
                        count++;
                    }
                    if (col - 1 >= 0 && isLandMineCell(col - 1, row)) {
                        count++;
                    }
                    if (col + 1 < BOARD_ROW_SIZE && isLandMineCell(col + 1, row)) {
                        count++;
                    }
                    if (row + 1 < BOARD_COL_SIZE && col - 1 >= 0 && isLandMineCell(col - 1, row + 1)) {
                        count++;
                    }
                    if (row + 1 < BOARD_COL_SIZE && isLandMineCell(col, row + 1)) {
                        count++;
                    }
                    if (row + 1 < BOARD_COL_SIZE && col + 1 < BOARD_ROW_SIZE && isLandMineCell(col + 1, row + 1)) {
                        count++;
                    }
    ...
    private static void open(int row, int col) {
        if (row < 0 || row >= BOARD_COL_SIZE || col < 0 || col >= BOARD_ROW_SIZE) {
            return;
        }
        if (!BOARD[row][col].equals(CLOSED_CELL_SIGN)) {
            return;
